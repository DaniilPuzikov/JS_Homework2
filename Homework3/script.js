...
document.addEventListener('click', (event) => {
    if (event.target.classList.contains('super_element')) {
        console.log(console.log(`Класс "super_element" присутствует в элементе "${event.target.tagName.toLowerCase()}".`))
    } else {
        console.log(console.log(`Класс "super_element" отсутствует в элементе "${event.target.tagName.toLowerCase()}".`))
    }
})
...
// 6
// button находится внутри тега ul, поэтому событие срабатывает сначала для родительского узла, 
// а затем для дочерних
